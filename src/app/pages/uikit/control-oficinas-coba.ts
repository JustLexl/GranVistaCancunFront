...
<p-drawer
    ...
    position="right"
    styleClass="!w-full sm:!w-[520px]"
    [style]="{ width: '100vw', maxWidth: '520px', background: '#f8fafc', borderLeft: '1px solid #e2e8f0' }"
    [modal]="true">
    
    <ng-template pTemplate="header">
        <div class="flex items-center gap-3 py-1">
            <span class="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold shrink-0">
    ...
    <div class="flex flex-col gap-5 p-1 pb-6">
        ...
        <div>
            <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Descripción de la Oficina</label>
            <textarea [(ngModel)]="formOficina.descripcion"
                rows="3"
                placeholder="Ej. Ubicada en el nivel 2, equipada con aire acondicionado y espacio para 6 escritorios..."
                class="w-full border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 bg-white resize-none"></textarea>
        </div>
    ...
    <div *ngIf="selectedOficina" class="flex flex-col gap-6 py-2">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-2xs text-sm text-slate-700 leading-relaxed whitespace-pre-line">
                    <span *ngIf="selectedOficina.descripcion && selectedOficina.descripcion.trim()">
                        {{ selectedOficina.descripcion }}
                    </span>
                    <span *ngIf="!selectedOficina.descripcion || !selectedOficina.descripcion.trim()" class="text-slate-400 italic text-xs block">
                        Sin descripción registrada para esta oficina.
                    </span>
                </div>
            </div>
...
export class ControlOficinasCobaComponent implements OnInit {
    ...
    saveOficina(): void {
        ...
        const payload: Omit<OficinaRecord, 'id' | '_id'> = {
            ...
            descripcion: this.formOficina.descripcion ? this.formOficina.descripcion.trim() : '',
            responsablePrincipal: {
